...
int board_smp_start(const unsigned cpu, void (*start)(void))
{
     ...
     */
    status = stm32_sec_firmware_psci(STM32_PSCI_CPU_ON_AARCH64, stm32mp2_core_affinity[cpu], (uintptr_t)start, 0x00, 0x00);
    if (status != STM32_PSCI_SUCCESS) {
        crash("Cortex-A core %d start failed! Status: 0x%llx. \n", cpu, status);
    }
    return 1;
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/hardware/startup/boards/stm32mp2/board_smp.c $ $Rev: 984580 $")
#endif
